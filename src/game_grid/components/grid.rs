...
impl Grid {
    pub fn new(width: u32, height: u32, node_size: f32) -> (Self, Vec<GridNode>) {
        let mut grid = Self { entries: vec![], positions: vec![], allocator: PositionAllocator { width: width, height: height, reserved_cells: vec![]}, node_size: node_size };
        let mut nodes = vec![];
    ...
    pub fn create_entity(&mut self, config: &GridEntityConfig) -> GridEntity {
        ...
        match self.allocator.allocate(width_with_box, height_with_box) {
            None => { panic!("not possible to allocate space in world for the object") },
            Some(allocation) => {
                ...
                for cur_x in allocation.x1..allocation.x2 {
                    for cur_y in allocation.y1..allocation.y2 {
                        let mut entry = self.entries.iter_mut().find(|entry| entry.x == cur_x && entry.y == cur_y).unwrap();

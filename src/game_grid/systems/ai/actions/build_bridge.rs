...
impl Action for BuildBridge {
    ...
    fn exec(&self, params: &mut PathfindingParams, state: &mut State) {
        ...
        Some(path) => {
            let bridge_positions = path.iter().filter(|node| params.grid.find_entity_type_by_coords(&node.0, &node.1) == Some(GridEntityType::Water));
        ...
        None => {
            params.graph_node_types.retain(|&item| item != GridEntityType::Water);
        }
    }
    }
}
